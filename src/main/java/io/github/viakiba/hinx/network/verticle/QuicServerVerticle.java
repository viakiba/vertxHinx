...
public class QuicServerVerticle extends AbstractVerticle implements BaseServerVerticle {
    ...
    public void start() throws Exception {
        JsonObject config = new JsonObject(configJson(NetType.quic));
        ...
        ChannelHandler codec = new QuicServerCodecBuilder().sslContext(context)
                .maxIdleTimeout(config.getInteger("idleTimeoutMs"), TimeUnit.MILLISECONDS)
                // Configure some limits for the maximal number of streams (and the data) that we want to handle.
                .initialMaxData(config.getInteger("maxData"))
                .initialMaxStreamDataBidirectionalLocal(config.getInteger("maxStreamDataBidirectional"))
                .initialMaxStreamDataBidirectionalRemote(config.getInteger("maxStreamDataBidirectional"))
                .initialMaxStreamsBidirectional(config.getInteger("maxStreamsBidirectional"))
                .initialMaxStreamsUnidirectional(config.getInteger("maxStreamsBidirectional"))
        ...
        Channel channel = bs.group(group)
                ...
                .handler(codec)
                .bind(new InetSocketAddress(config.getInteger("port"))).sync().channel();
        new Thread(() -> channel.closeFuture().syncUninterruptibly());
    }
