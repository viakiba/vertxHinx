...
public class ClassScanUtil {
    ...
    public static Set<Class<?>> getClzFromPkg(String pkg,Class anClazz) {
        Set<Class<?>> clzFromPkg = getClzFromPkg(pkg);
        Iterator<Class<?>> iterator = clzFromPkg.iterator();

        while (iterator.hasNext()){
            Class<?> next = iterator.next();
            Annotation an = next.getAnnotation(anClazz);
            if (null == an) {
                iterator.remove();
            }
        }
    ...
    public static Set<Class<?>> getClzFromPkg(String pkg) {
        ...
        try {
            Enumeration<URL> urls = ClassScanUtil.class.getClassLoader().getResources(pkgDirName);
